...
def normalize_vtt_for_players(vtt_content: str) -> str:
    """Replace Unicode glyphs that many players lack in their subtitle font."""
    if not vtt_content:
        return vtt_content
    return vtt_content.replace('\u2026', '...')
    ...
    try:
        if file_extension.lower() in ('ass', 'ssa'):
            ...
            try:
                ...
                return normalize_vtt_for_players(vtt_content)
                ...
                return normalize_vtt_for_players(subs.to_string('vtt'))
        else:
